...
| 导出补丁 | [export.html](export.html) | `pages/ExportPage.tsx` + `features/resources/components/patch-export/` | 已对齐：多选创建任务 → dry-run/强制构建 → 发布 → 任务列表完整链路；任务审计视图三栏卡片（DBC before→after / SQL 字段 / MPQ 清单，见 [07 §十二](../arch/07DBC维护与同步方案.md)）；🚧 规划扩展：混淆等级选择（none/basic/encrypted，见 [07 §十四](../arch/07DBC维护与同步方案.md)）、任务操作列（查看 MPQ / 删除） |
