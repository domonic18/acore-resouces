      ...
      <div className="flex items-center gap-1.5">
        {locked ? (
          <input
            type="number"
            disabled
            title={`已锁定，自动跟随：${linkedLabel}`}
            className={cn(
              "flex-1 cursor-not-allowed bg-bg-surface/60 text-text-tertiary",
              compact ? "form-input-compact" : "form-input",
            )}
            value={value === null || value === undefined ? "" : String(value)}
          />
        ) : (
          <NumberInput value={value} onChange={onChange} compact={compact} />
        )}
        <button
          type="button"
